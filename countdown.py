...
class CountDown:
    ...
    def create_widgets(self):
        try:
            title_label_police = main.fonts["countdown"]["police"]
        except Exception as e:
            title_label_police = 14

        self.title_label = tk.LabelFrame(self.root, text='Compte à rebours', font=('cambria', title_label_police, 'bold'), bg="white",
                                         fg="black")

        main.fonts["countdown"] = {"name": "Compte à rebours", "police": title_label_police,
        ...
        tk.Button(self.title_label, text="5 Secondes", command=lambda: self.submit(5),
                  font=("book antiqua", 13, "bold"), bg="#FDF2E9").grid(row=0, column=0, padx=0)
        tk.Button(self.title_label, text="10 Secondes", command=lambda: self.submit(10),
                  font=("book antiqua", 13, "bold"), bg="#FDF2E9").grid(row=0, column=1, padx=5)
        tk.Button(self.title_label, text="15 Secondes", command=lambda: self.submit(15),
                  font=("book antiqua", 13, "bold"), bg="#FDF2E9").grid(row=0, column=2, padx=5)
        tk.Button(self.title_label, text="30 Secondes", command=lambda: self.submit(30),
                  font=("book antiqua", 13, "bold"), bg="#FDF2E9").grid(row=0, column=3, padx=0)
