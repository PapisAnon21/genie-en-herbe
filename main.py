...
#com_arduino.init_com("com4",115200)
...
class Main:
    def __init__(self):
        ...
        self.create_widgets()
        self.joueurs()
        #ici j'appele la fonction comm_duino
        #try:
        #com_arduino.init_com("com4",115200)
        self.start_com()
        #except:
        #print('probleme avec la communication arduino')
    ...
    def create_widgets(self):
        ...
        btn_quit_match = tk.Button(text='quitter le match '.upper(), command=self.root.destroy,
                                   font=('calibri', btn_quit_match_police, 'bold'), bg='white', fg='IndianRed')
    ...
    def show_player_name(self,message_recu,color = "black"):
        if int(message_recu) >= 1 and int(message_recu) <= 4:
            j = tk.Label(self.grandlabel, text=JOUEURS[int(message_recu)-1], font=('book antiqua', 18, 'bold'), fg=color, bg="white", width=19)
            j.grid(row=2 + int(message_recu)-1, pady=3)
            #labs_joueurs.append(j)
            time.sleep(2)
            j.configure(fg='white')
        if  int(message_recu) >= 5 and int(message_recu) <= 8:
            j = tk.Label(self.grandlabel2, text=JOUEURS[int(message_recu)-1], font=('book antiqua', 18, 'bold'), fg=color, bg="white", width=19)
            j.grid(row=2 + int(message_recu)-1, pady=3)
            #labs_joueurs.append(j)
            time.sleep(2)
            j.configure(fg='white')
